                     ...
                     sstat_t *sstat)
{
    ...
    double iono,m_iono,c_iono=1.0-SQR(lam_carr[1]/lam_carr[0]);
    int i,j,k,sat,slip;

    for (i=0;i<n;i++) {
        P1=obs[i].P[0]; L1=obs[i].L[0]*lam_carr[0];
        P2=obs[i].P[1]; L2=obs[i].L[1]*lam_carr[1];
...
{
    double *sig,P1,P2,L1,L2,m_iono,c_iono=1.0-SQR(lam_carr[1]/lam_carr[0]);
    ...
    for (i=0;i<n;i++) {
        ...
        LG+=(lam_carr[0]-lam_carr[1])*phw[obs[i].sat-1];
...
static int est_iono(obs_t *obs, nav_t *nav, double *rr, FILE *fp)
{
    ...
    ekf_t *ekf;
    gtime_t time;
    double r,pos[3],rs[MAXOBS*6],dts[MAXOBS*2],var[MAXOBS],e[3],azel[MAXOBS*2];
    ...
    for (i=0;i<obs->n;i+=n) {
        ...
        for (j=0;j<n;j++) {
            if ((r=geodist(rs+j*6,rr,e))>0.0) satazel(pos,e,azel+j*2);
        ...
        /* ionosphere residuals */
        if ((nv=res_iono(obs->data+i,n,nav,rs,rr,pos,azel,NULL,ekf,phw,v,H,R))<=0) {
            continue;
        }
        /* filter */
        ...
        out_iono(obs->data[i].time,ekf,sstat,pos,fp);
    }
