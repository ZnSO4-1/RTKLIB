...
static int raw_obs(const obsd_t *obs, const nav_t *nav, double *P1, double *P2,
                   double *L1, double *L2)
{
    double gamma=SQR(lam_carr[0]/lam_carr[1]);
    
    *L1=obs->L[0]*lam_carr[0];
    *L2=obs->L[1]*lam_carr[1];
    *P1=obs->P[0];
    *P2=obs->P[1];
                     ...
                     sstat_t *sstat)
{
    ...
    double x[2]={0},P[2],c_iono=1.0-SQR(lam_carr[1]/lam_carr[0]);
    int i,sat,slip;
...
{
    double *sig,P1,P2,L1,L2,c_iono=1.0-SQR(lam_carr[1]/lam_carr[0]);
