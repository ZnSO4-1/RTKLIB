...
*
* notes   :
*     upstream version does not support win32 without pthread library
*     this downstream C++ migration adds a minimal MSVC/Win32 compatibility path
*     for local command-line build verification
*
* 说明   :
*     上游版本在没有 pthread 库时不支持 win32
*     本下游 C++ 迁移为本地命令行编译验证补充了最小 MSVC/Win32 兼容路径
*
...
#ifdef WIN32
static DWORD WINAPI sendkeepalivewin(void *arg)
{
    sendkeepalive(arg);
    return 0;
}
...
static int openmoni(int port)
{
    ...
    keepalive=1;
#ifdef WIN32
    if (!(thread=CreateThread(NULL,0,sendkeepalivewin,NULL,0,NULL))) {
        keepalive=0;
        strclose(&moni);
        return 0;
    }
...
static void closemoni(void)
{
    ...
    strwrite(&moni,(unsigned char *)MSG_DISCONN,(int)strlen(MSG_DISCONN));
...
static void prstatus(vt_t *vt)
{
    ...
    rtcm_t rtcm[3];
    int i,j,n,cycle,state,rtkstat,nsat0,nsat1,prcout;
    uintptr_t thread;
    ...
    vt_printf(vt,"%-28s: %zu\n","rtk server thread",(size_t)thread);
