...
{
    fprintf(stderr,"telnet console is not supported in this MSVC build: port=%d\n",port);
    return -1;
}
#else
/* accept client socket ------------------------------------------------------*/
...
static int clear_buff(vt_t *vt)
{
    ...
    int i,len=(int)strlen(vt->buff);
    for (i=0;i<vt->cur;i++) *p++='\b';
    for (i=0;i<len;i++) *p++=' ';
    for (i=0;i<len;i++) *p++='\b';
    vt->n=vt->nesc=vt->cur=0;
    return write(vt->out,buff,(unsigned int)(p-buff))==(int)(p-buff);
}
/* refresh line buffer -------------------------------------------------------*/
...
{
    int i,len;
    if ((len=(int)strlen(buff))<=0) return 1;
...
#ifdef WIN32
    if ((stat=read(vt->in,c,1))!=1) return stat==0;
#else
    FD_ZERO(&rs);
    FD_SET(vt->in,&rs);
...
extern int vt_puts(vt_t *vt, const char *buff)
{
    ...
    for (p=buff;(q=strchr(p,'\n'));p=q+1) {
        if (!vt_putchar(vt,p,(int)(q-p))||!vt_putchar(vt,"\r\n",2)) return 0;
    }
    return vt_putchar(vt,p,(int)strlen(p));
}
