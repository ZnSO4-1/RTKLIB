...
static serial_t *openserial(const char *path, int mode, char *msg)
{
    ...
    const char *p;
    char parity='N',dev[128],port[128],fctr[64]="";
#ifdef WIN32
...
static int readfile(file_t *file, unsigned char *buff, int nmax, char *msg)
{
    ...
    if (nmax>0) {
        nr=(unsigned int)fread(buff,1,nmax,file->fp);
        file->fpos+=nr;
...
static int writefile(file_t *file, unsigned char *buff, int n, char *msg)
{
    ...
    ns=(unsigned int)fwrite(buff,1,n,file->fp);
    fpos=(unsigned int)ftell(file->fp);
    fflush(file->fp);
    file->wtime=wtime;
    
    if (file->fp_tmp) {
        fwrite(buff,1,n,file->fp_tmp);
        fpos_tmp=(unsigned int)ftell(file->fp_tmp);
        fflush(file->fp_tmp);
    }
    if (file->fp_tag) {
        tick-=file->tick;
...
#ifdef WIN32
    if (!select(0,&rs,NULL,NULL,&tv)) return 0;
#else
...
#ifdef WIN32
    if (!select(0,NULL,&ws,NULL,&tv)) return 0;
#else
...
static int reqntrip_s(ntrip_t *ntrip, char *msg)
{
    ...
    if (writetcpcli(ntrip->tcp,(unsigned char *)buff,(int)(p-buff),msg)!=(int)(p-buff)) return 0;
